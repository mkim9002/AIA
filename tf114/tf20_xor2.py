...
# model.add(Dense(7))
w2 = tf.compat.v1.Variable(tf.compat.v1.random_normal([10, 7], dtype=tf.float32), name= 'weight')  #weight는 행열연산 해줘야하므로 shape맞춰주기 [x*w = y(hy)] #w의 shape 
...
w3 = tf.compat.v1.Variable(tf.compat.v1.random_normal([7, 1], dtype=tf.float32), name= 'weight')  #weight는 행열연산 해줘야하므로 shape맞춰주기 [x*w = y(hy)] #w의 shape 
...
#3. 컴파일, 훈련 
#3-1. 컴파일
cost = -tf.reduce_mean(y*tf.log1p(hypothesis) + (1-y)*tf.log1p(1-hypothesis))
