...
meta_data = pd.read_csv(path + 'meta_all.csv')
test_data = pd.read_csv(path + 'test_aws_all.csv')
train_data = pd.read_csv(path + 'train_aws_all.csv')
...
categorical_cols = ['연도', '일시', '지점']
label_encoder = LabelEncoder()
for col in categorical_cols:
    all_data[col] = label_encoder.fit_transform(all_data[col])

# Merge with meta_data based on the common column '지점'
all_data = pd.merge(all_data, meta_data, on='지점', how='left')
...
# Train the model
model.fit(X_train_scaled, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=[EarlyStopping(patience=5)])
...
# Save the results
submission.to_csv(save_path + 'submit5.csv', index=False)
