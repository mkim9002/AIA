...
X_train = all_data[:train_data.shape[0]].drop(['PM2.5'], axis=1).values.astype(float)
y_train = train_data['PM2.5'].values.astype(float)

X_test = all_data[train_data.shape[0]:].drop(['PM2.5'], axis=1).values.astype(float)
...
# Reindex the submission dataframe to match the length of the predicted values
