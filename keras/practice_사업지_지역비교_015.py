...
# 1.2 데이터 범위 설정,전처리
# 1.2.1 drop 설정
검색수_x = np.array(datasets_검색수.drop(['2018'], axis=1))
검색수_y = np.array(datasets_검색수['시도명'])
방문자수_x = np.array(datasets_방문자수.drop(['2018'], axis=1))
방문자수_y = np.array(datasets_방문자수['시도명'])
...
print(검색수_x.shape, 검색수_y.shape)
print(방문자수_x.shape, 방문자수_y.shape)
...
print(검색수_x_train_split.shape)      # (820, 20, 14)
print(방문자수_x_train_split.shape)      # (820, 20, 14)
...
model.save(path_save + 'keras53_검색수2_kmg.h5')
...
검색수_x_predict = 검색수_x_predict.reshape(1, timesteps, 14)
방문자수_x_predict = 방문자수_x_test[-timesteps:]
방문자수_x_predict = 방문자수_x_predict.reshape(1, timesteps, 14)

predict_result = model.predict([검색수_x_predict, 방문자수_x_predict])

print("시도명 : ", np.round(predict_result[0], 2))
